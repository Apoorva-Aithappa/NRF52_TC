...
{
  NRF_TIMER1->PRESCALER = 4;	// 16/(2^4) gives timer of resolution 1 micro second 

  NRF_TIMER1->TASKS_STOP = 1;	// Stop timer
  NRF_TIMER1->TASKS_CLEAR = 1; // Clear timer
  NRF_TIMER1->MODE = TIMER_MODE_MODE_Timer; // Setiing TIMER mode to timer --->0x504 offset
  NRF_TIMER1->BITMODE = (TIMER_BITMODE_BITMODE_24Bit << TIMER_BITMODE_BITMODE_Pos); //setting the bitmode of the timer
  
  NRF_TIMER1->CC[0] = trigger_interval * 1000;  // meeting decimal requirement as the input is of millisecond resolution
  NRF_TIMER2->INTENSET = (TIMER_INTENSET_COMPARE0_Enabled << TIMER_INTENSET_COMPARE0_Pos) | (TIMER_INTENSET_COMPARE1_Enabled << TIMER_INTENSET_COMPARE1_Pos); // recommonded to do as per some forum comments (for nrf52 series)
